...
function urlClassify(url) {
  ...
  for (const [prefix, subcode, thematic, clusterOverride] of URL_RULES) {
    ...
    if (subcode !== null) {
      if (!tid.includes(`-${subcode}-`) && !tid.endsWith(`-${subcode}`)) {
        continue;
      }
    }
    return {
...
async function fetchPage(page) {
  ...
  params.append("query/status/code", "31094501");
  params.append("query/status/code", "31094502");
  params.append("query/programmePeriod/code", "2021 - 2027");

  const url = `${EU_API}?${params.toString()}`;
  ...
  // Fallback: se GET viene bloccata, prova POST
  // ATTENZIONE: apiKey deve stare nell'URL, non nel body JSON
  if (resp.status === 405) {
    console.log(`GET blocked (405), retrying with POST on page ${page}...`);

    resp = await fetch(`${EU_API}?apiKey=${encodeURIComponent(API_KEY)}`, {
      method: "POST",
      headers: {
        ...headers,
...
async function fetchAll() {
  ...
  // fetch pagine successive con una piccola pausa
  for (let p = 2; p <= pages; p++) {
    process.stdout.write(`Fetching page ${p}/${pages}...\r`);
